    ...
    def next(self):
        ...
        for data in self.datas:
            if (
                ...
                close_df[data._name] = data.close.get(0, 200)  # 30 data points
        ...
        if self.t % 2 == 0:
            ...
            if not _df.empty:
                try:
                    weights = self.MPT_minvol(_df)
                    print(weights, "gggggggggggggg")
                except Exception as e:
                    print(e)
                    weights = None

            if (not _to_buy) or (not weights):
                ...
                for ticker in self.buy_value.keys():
                    if ticker in _to_buy:
                        self.buy_value[ticker] = (
                            weights[ticker] * self.broker.get_value() * 0.95
                        )
                    else:
