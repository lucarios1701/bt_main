...
class MetaStrategy(StrategyBase.__class__):
    ...
    def dopreinit(cls, _obj, *args, **kwargs):
        ...
        # @tuando: add broker of cerebro to Strategy object
...
class Strategy(with_metaclass(MetaStrategy, StrategyBase)):
    ...
    def _oncepost(self, dt):
        ...
        if (
            minperstatus < 0
        ):  # @tuando: guess: minperstatus will < 0 means there still have data to loop next
            self.next()
        elif minperstatus == 0:
    ...
    def _addnotification(self, order, quicknotify=False):
        ...
        for exbit in order.executed.iterpending():
            ...
            if (
                exbit.closed
            ):  # @tuando: exbit.closed when the position update an close the position
                trade.update(
                    order,
                    exbit.closed,
                    exbit.price,
            ...
            if (
                exbit.opened
            ):  # @tuando: exbit.opened when the position update and open new position
                if (
                    trade.isclosed
                ):  # @tuando: if new exbit is open and the old trade isclosed, then creat the new Trade
                    trade = Trade(
                        data=tradedata,
    ...
    #  _brokernotify which had broker 'next'
    def _notify(self, qorders=[], qtrades=[]):
        ...
        for order in procorders:
            if (
                order.exectype != order.Historical or order.histnotify
            ):  # @tuando: doesn't verify this condition yet
