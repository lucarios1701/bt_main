    ...
    *cash* is adjusted on each iteration for instruments like ``futures`` for
    ...
    def check_submitted(self):
        ...
        while self.submitted:
            ...
            )  # @tuando: this will also add data to self.postions attr
    ...
    def _execute(
        ...
        if self.p.filler is None or ago is None:
            ...
            if not order.isbuy():
                # @tuando: by default order will execute to next open because
                # @tuando: self.p.coo and coc is False
        ...
        if opened:
            ...
            if cash < 0.0:
                ...
                if abs(psize) > abs(
                    opened
                ):  # @tuando - guess: if not future, this hasn't used yet
    ...
    def _try_exec_market(self, order, popen, phigh, plow):
        ...
        if self.p.coc and order.info.get("coc", True):
            ...
            dtcoc = None
            # @tuando: if we cheat on open the buy price will be the next open price
            exprice = popen
    ...
    def _try_exec(self, order):
        ...
        # @tuando: tick_open was hided by 'tick_' + O/H/L/C in data's class,
        # @tuando - it was created by _tick_nullify() 'feed.py' then advance will call
        ...
        ):  # @tuando: if exectype = None, it will be Order.market
    ...
    def next(self):
        ...
        if self.p.checksubmit:
            # @tuando - guess: this will check submitted by check the cash
            # @tuando - balance, if pass the order will be added to pending
            # @tuando - order an real execute after
