...
class MetaLineIterator(LineSeries.__class__):
    def donew(cls, *args, **kwargs):
        ...
        # @tuando: This _obj.datas will append data if have the root is LineRoot,
        # @tuando - dont have to specify variable name
        for arg in args:
    ...
    def dopostinit(cls, _obj, *args, **kwargs):
        ...
        # @tuando: this will update the _minperiod of
        # self._lineiterators[LineIterator.IndType] in _periodset of Strategy
        # @tuando: We need the _minperiod of lines[0] of 'sma' is updated in
        # dopreinit of _LineDelay, then the x._minperiod here will be update
        # for sma
        _obj._minperiod = max([x._minperiod for x in _obj.lines])

        # Recalc the period
        # @tuando: this will also recalc update the _minperiod of
        # self._lineiterators[LineIterator.IndType] in _periodset of Strategy
        _obj._periodrecalc()
...
class LineIterator(with_metaclass(MetaLineIterator, LineSeries)):
    ...
    def _once(self):
        ...
        for (
            ...
            self.datas
        ):  # This is 'datas' of strategy class was added when call 'strat' in cerebro
        ...
        # @tuando: this 'once' will calculated required data (e.g Average) for
        # @tuando - indicators (e.g SMA)
    ...
    def once(
        self, start, end
    ):  # @tuando - guess: this once used for Indicator (Average e.g)
        pass
