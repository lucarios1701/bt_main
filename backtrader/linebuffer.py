...
class LineBuffer(LineSingle):
    ...
    def reset(self):  # @tuando: this will reset the array of line to empty
    ...
    # @tuando: forward will create an 'NaN' array with needed lenght for saving data after
    def forward(self, value=NAN, size=1):
        ...
        '''
        self.idx += size  # @tuando: paralel save the index of data in self.array
        self.lencount += size
        for i in range(size):
            self.array.append(value)

    def backwards(self, size=1, force=False):  # @tuando
        ...
        self.lencount -= size
        for i in range(size):  # @tuando: if the 'size' greater than 1, remove all of that new points
            self.array.pop()
