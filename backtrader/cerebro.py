...
class Cerebro(with_metaclass(MetaParams, object)):
    ...
    def __call__(self, iterstrat):  # @tuando: only used when optimization
        """
    ...
    def run(self, **kwargs):
        ...
        pkeys = self.params._getkeys()  # @tuando note: get params key from _getpairs()
        ...
        # @tuando: self.strats was appended 'strategy, args and kwargs'
        # @tuando: this will drop the bracket of self.strats
    ...
    def _next_stid(self):
        # @tuando: next was counted in metaclass of Strategy
    ...
    def runstrategies(self, iterstrat, predata=False):
        ...
        if not predata:
            ...
            for data in self.datas:
                ...
                if (
                    self._dopreload
                ):  # @tuando: preload is True and will be called if not live trading
                    data.preload()

        for stratcls, sargs, skwargs in iterstrat:
            # @tuando: self.datas is an data class (e.g. YahooCSV...,) later will be load to Lines's class to add data
    ...
    def _runonce(self, runstrats):
        ...
        datas = sorted(
            self.datas, key=lambda x: (x._timeframe, x._compression)
        )  # @tuando: this data had data already because of loaded before
